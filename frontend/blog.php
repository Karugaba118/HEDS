<!-- starter banner/ breadcrumb start -->
<div class="breadcrumb-header" style="  background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5) ), url('../img/blog/2.png'); background-size: cover;">
    <div class="container">
        <div class="row">
            <div class="col-md-12 content">
                <h2 data-aos="slide-down" data-aos-duration="2000"><span style="color: #ff9900">Blog</span></h2>
                <h5 class="text-center breadcrumb-intro" data-aos="slide-up" data-aos-duration="1000">Discover latest trends</h5>
                <ul class="breadcrumb justify-content-center">
                    <li><a href="index.php">Home</a></li>
                    <li>Blog</li>
                </ul>
            </div>
        </div>
    </div>
</div>
<!-- starter banner/ breadcrumb end -->

<!-- blog start -->
<section class="modern-blog">
  <div class="container">
    <div class="text-center mb-5">
      <h2 class="blog-main-title">Latest <span class="accent-text">Insights</span></h2>
      <p class="blog-subtitle">Discover the latest trends and strategies in digital innovation</p>
    </div>

    <div class="row">
      <!-- Featured Post -->
      <div class="col-lg-12 mb-5">
        <div class="featured-post">
          <div class="row align-items-center">
            <div class="col-md-6">
              <div class="featured-image">
                <img src="../img/blog/RWD.png" alt="Featured Post" class="img-fluid">
                <div class="post-badge">Featured</div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="featured-content">
                <div class="post-meta">
                  <span class="category">Web Development</span>
                  <span class="date"><i class="far fa-calendar"></i> June 20, 2025</span>
                </div>
                <h3><a href="#!blog-detail.php">The Future of Responsive Web Design in 2025</a></h3>
                <p>Explore how responsive design continues to evolve with new technologies and user expectations shaping the digital landscape.</p>
                <a href="#!blog-detail.php" class="read-more-btn">Read Full Article <i class="fas fa-arrow-right"></i></a>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- Regular Posts -->
      <div class="col-md-6 col-lg-4 mb-4">
        <div class="blog-card">
          <div class="card-image">
            <img src="../img/blog/cross-platform.png" alt="Blog Post" class="img-fluid">
            <div class="image-overlay"></div>
            <div class="category-tag">App Development</div>
          </div>
          <div class="card-content">
            <div class="meta-info">
              <span><i class="far fa-user"></i> Sarah Johnson</span>
              <span><i class="far fa-clock"></i> 5 min read</span>
            </div>
            <h4><a href="blog-detail.html">Cross-Platform Development: Flutter vs React Native</a></h4>
            <p>Comparing the two leading frameworks for building cross-platform mobile applications in 2025.</p>
            <a href="blog-detail.html" class="card-link">Continue Reading <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      </div>

      <div class="col-md-6 col-lg-4 mb-4">
        <div class="blog-card">
          <div class="card-image">
            <img src="../img/blog/minimalistic-design.png" alt="Blog Post" class="img-fluid">
            <div class="image-overlay"></div>
            <div class="category-tag">UI/UX Design</div>
          </div>
          <div class="card-content">
            <div class="meta-info">
              <span><i class="far fa-user"></i> Michael Chen</span>
              <span><i class="far fa-clock"></i> 4 min read</span>
            </div>
            <h4><a href="blog-detail.html">Minimalist Design Principles for Modern Interfaces</a></h4>
            <p>How simplicity and clarity in design can enhance user experience and engagement.</p>
            <a href="blog-detail.html" class="card-link">Continue Reading <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      </div>

      <div class="col-md-6 col-lg-4 mb-4">
        <div class="blog-card">
          <div class="card-image">
            <img src="../img/blog/seo-strategy.png" alt="Blog Post" class="img-fluid">
            <div class="image-overlay"></div>
            <div class="category-tag">Digital Marketing</div>
          </div>
          <div class="card-content">
            <div class="meta-info">
              <span><i class="far fa-user"></i> Emily Roberts</span>
              <span><i class="far fa-clock"></i> 6 min read</span>
            </div>
            <h4><a href="blog-detail.html">SEO Strategies That Actually Work in 2025</a></h4>
            <p>Data-driven approaches to improve your search engine rankings and organic traffic.</p>
            <a href="blog-detail.html" class="card-link">Continue Reading <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      </div>

      <div class="col-md-6 col-lg-4 mb-4">
        <div class="blog-card">
          <div class="card-image">
            <img src="../img/blog/check-out.png" alt="Blog Post" class="img-fluid">
            <div class="image-overlay"></div>
            <div class="category-tag">E-Commerce</div>
          </div>
          <div class="card-content">
            <div class="meta-info">
              <span><i class="far fa-user"></i> David Wilson</span>
              <span><i class="far fa-clock"></i> 7 min read</span>
            </div>
            <h4><a href="blog-detail.html">Optimizing Checkout Processes for Higher Conversions</a></h4>
            <p>Reducing cart abandonment through streamlined checkout experiences.</p>
            <a href="blog-detail.html" class="card-link">Continue Reading <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      </div>

      <div class="col-md-6 col-lg-4 mb-4">
        <div class="blog-card">
          <div class="card-image">
            <img src="../img/blog/brand-identity.png" alt="Blog Post" class="img-fluid">
            <div class="image-overlay"></div>
            <div class="category-tag">Branding</div>
          </div>
          <div class="card-content">
            <div class="meta-info">
              <span><i class="far fa-user"></i> Lisa Anderson</span>
              <span><i class="far fa-clock"></i> 5 min read</span>
            </div>
            <h4><a href="blog-detail.html">Building a Consistent Brand Identity Across Platforms</a></h4>
            <p>Maintaining brand coherence in a multi-channel digital environment.</p>
            <a href="blog-detail.html" class="card-link">Continue Reading <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      </div>

      <div class="col-md-6 col-lg-4 mb-4">
        <div class="blog-card">
          <div class="card-image">
            <img src="../img/blog/modern-web.png" alt="Blog Post" class="img-fluid">
            <div class="image-overlay"></div>
            <div class="category-tag">Web Development</div>
          </div>
          <div class="card-content">
            <div class="meta-info">
              <span><i class="far fa-user"></i> Alex Turner</span>
              <span><i class="far fa-clock"></i> 8 min read</span>
            </div>
            <h4><a href="blog-detail.html">The Rise of Jamstack: Modern Web Architecture</a></h4>
            <p>Understanding the benefits and implementation of Jamstack for modern websites.</p>
            <a href="blog-detail.html" class="card-link">Continue Reading <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      </div>
    </div>

  </div>
</section>
<!-- blog end -->